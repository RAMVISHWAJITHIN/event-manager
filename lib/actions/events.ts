...
//     {
//         throw new Error("title must be between 3 and 120 character")
...
//     return (RSVP_STATUS as readonly string[]).includes(s)
// }
...
//         throw new Error("Nam emust be between 2 and 120 characters")
...
export async function createEventAction(formData: FormData) {
  ...
  redirect(`/events/${created.id}`); // ✅ leave it alone
  } catch (error) {
    console.error(error)
  }
}
